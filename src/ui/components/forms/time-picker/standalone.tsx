...
function generateMinutes(interval: number): string[] {
  const result: string[] = []
  for (let m = 0; m < 60; m += interval) result.push(String(m).padStart(2, '0'))
  return result
}
...
function formatTimeDisplay(hour: number, minute: number, is24Hour: boolean): string {
  if (is24Hour) {
...
/**
 * Standalone TimePicker — wheel-style time entry in a modal.
 *
 * @example
 * <TimePickerBase label="Reminder" onChange={setTime} />
 */
export function TimePickerBase({
  ...
  const parsedDefault = defaultValue ? parseTime(defaultValue) : null
  ...
  const handleConfirm = useCallback(() => {
    ...
    onChange?.(formatTimeValue(actual24Hour, selectedMinute))
  }, [actual24Hour, selectedMinute, onChange])
            ...
            <TouchableOpacity activeOpacity={1} accessible={false}>
              ...
              <View style={styles.columnsRow}>
                <ScrollColumn
                  ...
                  tokens={tokens}
                  testIDPrefix={`${testIDBase ?? 'time-picker'}-hour`}
                />
                <Text style={styles.separator}>:</Text>
                <ScrollColumn
                  items={minutes}
                  selectedIndex={minuteIndex}
                  onSelect={(index) => setSelectedMinute(index * minuteInterval)}
                  tokens={tokens}
                  testIDPrefix={`${testIDBase ?? 'time-picker'}-minute`}
                />
                ...
                {!is24Hour && (
                  <View style={styles.periodColumn}>
                    <TouchableOpacity
                      ...
                      testID={testIDBase ? `${testIDBase}-am` : undefined}
                    >
                    ...
                    <TouchableOpacity
                      ...
                      testID={testIDBase ? `${testIDBase}-pm` : undefined}
                    >
...
function makeStyles(tokens: DesignTokens) {
  return StyleSheet.create({
    ...
    clockIcon: { fontSize: tokens.typography.fontSizeMd, marginLeft: tokens.spacing[2] },
    backdrop: {
      flex: 1,
    ...
    periodColumn: { gap: tokens.spacing[2], marginLeft: tokens.spacing[3] },
    periodButton: {
      paddingHorizontal: tokens.spacing[3],
    ...
    periodTextActive: { color: tokens.colors.primaryForeground },
    footer: {
      paddingHorizontal: tokens.spacing[4],
      paddingVertical: tokens.spacing[3],
      borderTopWidth: 1,
...
function makeColumnStyles(tokens: DesignTokens) {
  return StyleSheet.create({
    columnContainer: { height: ITEM_HEIGHT * 5, width: 64, position: 'relative' },
    highlight: {
      position: 'absolute',
      top: ITEM_HEIGHT * 2,
      left: 0,
    ...
    itemText: { fontSize: tokens.typography.fontSizeLg, color: tokens.colors.textMuted },
    itemTextSelected: {
      color: tokens.colors.text,
      fontWeight: tokens.typography.fontWeightBold,
    },
