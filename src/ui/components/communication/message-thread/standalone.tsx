...
export interface MessageThreadBaseProps {
  ...
  /** Current user id (used for own bubble alignment). */
  ...
  /** Error state. */
  error?: boolean
  /** Called when scrolled to end (load more older). */
  onLoadMore?: () => void
  /** Called when user taps reply on a message. */
  onReply?: (message: MessageThreadMessage) => void
...
function StatusText({ status, color }: { status: MessageStatus | undefined; color: string }) {
...
function MiniReactions({
  ...
  reactions: NonNullable<MessageThreadMessage['reactions']>
  tokens: DesignTokens
}) {
  return (
...
/**
 * Standalone MessageThread — plain React props, no manifest required.
 *
 * @example
 * <MessageThreadBase currentUserId="u1" messages={[]} onReply={(m) => setReplyTo(m)} />
 */
export function MessageThreadBase({
  ...
  const renderItem = useCallback(
    ...
    [showAvatars, tokens, styles, handleLongPress, onReply],
  )
  ...
  if (loading) {
    return <MessageSkeleton tokens={tokens} />
  }

  if (error) {
    return (
      <View style={[styles.errorContainer, style]} testID={testID ?? id}>
...
function makeStyles(tokens: DesignTokens) {
  return StyleSheet.create({
    ...
    errorText: { fontSize: tokens.typography.fontSizeSm, color: tokens.colors.error },
  })
}
